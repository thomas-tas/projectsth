...
def J_b3(x, y):
    ...
    J = 2 * (a3 - y)  # dC/da3
    J = J * d_sigma(z3)  # Multiply by d_sigma(z3)
...
def J_W1(x, y):
    ...
    J = J * d_sigma(z3)  # Multiply by d_sigma(z3)
    J = (J.T @ W3).T  # da3/da2
    J = J * d_sigma(z2)  # Multiply by d_sigma(z2)
    J = (J.T @ W2).T  # da2/da1
    J = J * d_sigma(z1)  # Multiply by d_sigma(z1)
    J = J @ a0.T / x.size  # Final step for dC/dW1
    return J
...
def J_b1(x, y):
    ...
    J = (J.T @ W2).T  # da2/da1
    J = J * d_sigma(z1)  # Multiply by d_sigma(z1)
    J = np.sum(J, axis=1, keepdims=True) / x.size  # Final step for dC/db1
    return J
